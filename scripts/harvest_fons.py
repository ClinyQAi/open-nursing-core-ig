...
def harvest():
    ...
    print("🚀 Downloading FULL PDF Library...")
    try:
        soup = BeautifulSoup(
            requests.get(START_URL, headers=HEADERS).text,
            "html.parser"
        )
        links = {
            urljoin(BASE_URL, a['href'])
            for a in soup.find_all('a', href=True)
            if '/library/journal/volume' in a['href']
        }

        count = 0
        for i, link in enumerate(links):
            print(f"   Scanning Issue {i+1}/{len(links)}...", end="\r")
            try:
